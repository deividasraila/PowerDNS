...
static void declareArguments()
{
  ...
  ::arg().set("dnsproxy-threads", "Number of DNS Proxy shard threads handling ALIAS expansion (0 = match receiver-threads)") = "4";
  ::arg().set("dnsproxy-timeout", "Milliseconds to wait for a recursor reply before reusing the slot for an ALIAS lookup") = "1400";
...
static void declareStats()
{
  ...
  S.declare("recursing-slot-exhaustion", "Number of ALIAS lookups dropped because all DNS Proxy shard slots were in use");
  S.declare("recursing-duplicate-replies", "Number of duplicate recursor replies the DNS Proxy received");
